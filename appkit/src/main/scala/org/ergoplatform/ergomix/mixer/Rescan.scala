...
class Rescan(tables: Tables) {
  ...
  def processRescan(mixId:String, round:Int, goBackward:Boolean, isHalfMixTx:Boolean, mixBoxId:String) = {
    ...
    val followedMixes: Seq[FollowedMix] = if (!goBackward) { // go forward
      ...
      MixScanner.followDeposit(mixBoxId, masterSecret)
    }
  ...
  def getFollowedMix(mixId:String): Seq[FollowedMix] = {
    mixRequestsTable.select(depositAddressCol, masterSecretCol).where(mixIdCol === mixId).as(a => (a(0).as[String], a(1).as[BigDecimal])).headOption.flatMap{
      case (depositAddress, bigDecimal) =>
        spentDepositsTable.select(boxIdCol).where(addressCol === depositAddress).firstAsT[String].headOption.map(boxId => MixScanner.followDeposit(boxId, bigDecimal.toBigInt()))
