    ...
    if (halfMixBoxConfirmations >= minConfirmations) {
      ...
      if (halfMixBoxConfirmations == 0) { // 0 confirmations for halfMixBoxId
        ...
        optEmissionBoxId match {
          ...
          case _ => // no emission box, so this is an entry. Need to check for fork by checkinf if any of the deposits have suddenly disappeared
            spentDepositsTable.select(boxIdCol).where(mixIdCol === mixId).firstAsT[String].find{depositBoxId =>
